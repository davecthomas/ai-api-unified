...
if bedrock_available:
    ...
    def test_nova_canvas_image_properties_valid_dimensions(
        width: int, height: int
    ) -> None:
        props = AINovaCanvasImageProperties(width=width, height=height, num_images=2)
    ...
    def test_nova_canvas_generate_images() -> None:
        ...
        image_client = AIFactory.get_ai_images_client(
        ...
        root_file_path: Path = (
            image_files_path / f"test_generate_image_files_{date_str}"
        )

        prompt = "Generate a trippy abstract pattern with soft colors."
        props = AINovaCanvasImageProperties(width=704, height=320, num_images=1)
