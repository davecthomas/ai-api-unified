...
class AIVoiceAzure(AIVoiceBase):
    ...
    def __init__(self, *, engine: str, **kwargs: Any) -> None:
        ...
        object_api_key: object = env_settings.get_setting(
        ...
        object_region: object = env_settings.get_setting(
            "MICROSOFT_COGNITIVE_SERVICES_REGION"
        )
        region: str | None = object_region if isinstance(object_region, str) else None
        if not region:
    ...
    def _build_voice_catalog(self) -> list[AIVoiceSelectionAzure]:
        ...
        try:
            ...
            for voice in voices_result.voices:
                if voice.locale in self.common_vendor_capabilities.supported_locales:
                    catalog.append(
    ...
    def _synthesize_audio_bytes(
        ...
        *,
        ...
        """
        Execute one Azure TTS request and return the provider audio bytes.

        Args:
            text_to_convert: Input text that should be synthesized into speech.
            voice: Resolved Azure voice used for the synthesis request.
            audio_format: Caller-facing audio format configured on the Azure SDK client.
            speaking_rate: Requested speaking-rate multiplier supplied by the caller.
            use_ssml: True when the Azure synthesis call should use SSML.
            max_attempts: Maximum number of Azure synthesis attempts before surfacing failure.
            initial_backoff_seconds: Initial retry delay used for non-rate-limit retries.
            backoff_multiplier: Exponential multiplier applied to retry delays.

        Returns:
            Raw audio bytes returned by the Azure SDK for the completed synthesis request.
        """
        if speaking_rate != 1.0 and not use_ssml:
            raise ValueError("speaking_rate different from 1.0 requires use_ssml=True")
        ...
        if use_ssml:
            ...
            def invoke_synthesis() -> speechsdk.SpeechSynthesisResult:
                return self.client.speak_ssml_async(payload).get()

        else:
            payload = text_to_convert

            def invoke_synthesis() -> speechsdk.SpeechSynthesisResult:
                return self.client.speak_text_async(payload).get()
        ...
        while attempt_index < max_attempts:
            attempt_index += 1
            try:
                synthesis_result: speechsdk.SpeechSynthesisResult = invoke_synthesis()

                if (
                    synthesis_result.reason
                    == speechsdk.ResultReason.SynthesizingAudioCompleted
                ):
                    # Normal return with the synthesized Azure audio payload.
    ...
    def stream_audio(
        ...
        selected_voice: AIVoiceSelectionAzure = (
