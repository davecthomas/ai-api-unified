...
from ..util import EnvSettings, is_hex_enabled
...
class AIVoiceElevenLabs(AIVoiceBase):
    ...
    def __init__(self, *, engine: str, **kwargs: Any) -> None:
        ...
        object_api_key: object = env_settings.get_setting("ELEVEN_LABS_API_KEY", "")
        api_key: str = object_api_key if isinstance(object_api_key, str) else ""
        if not api_key:
    ...
    def _list_voices_internal(
        ...
        while True:
            for attempt in range(1, max_retries + 1):
                try:
                    page_voice_search_resp: GetVoicesV2Response = self._search_voices(
                        page_size=100,
                        next_page_token=next_page_token,
                    )
                except ConnectTimeout:
        ...
        audio_bytes: bytes = self._execute_voice_call_with_observability(
            ...
            callable_build_result_summary=lambda output_audio_bytes, provider_elapsed_ms: self._build_tts_observability_result_summary(
                observed_result=AiApiObservedTtsResultModel(
                    ...
                    dict_metadata={
                        "output_audio_format": audio_format.key,
                        "custom_voice_settings_supplied": voice_settings is not None,
                    },
                ),
    ...
    def _synthesize_audio_bytes(
        ...
        *,
        text_to_convert: str,
        voice: AIVoiceSelectionElevenLabs,
        audio_format: AudioFormat,
        voice_settings: dict[str, Any] | None,
    ) -> bytes:
        """
        Execute one ElevenLabs TTS request and return the provider audio bytes.

        Args:
            text_to_convert: Input text that should be synthesized into speech.
            voice: Resolved ElevenLabs voice used for the synthesis request.
            audio_format: Caller-facing audio format requested from ElevenLabs.
            voice_settings: Optional ElevenLabs-specific voice settings passed through unchanged.

        Returns:
            Audio bytes returned by ElevenLabs for the completed synthesis request.
        """
        model_id: str = (
            self.selected_model.name if self.selected_model else self.default_model_id
        )
        raw_audio: bytes | Iterator[bytes] = self.client.text_to_speech.convert(
        ...
        if isinstance(raw_audio, Iterator):
            combined_audio_bytes: bytes = b"".join(raw_audio)
            # Early return with the aggregated iterator payload returned by ElevenLabs.
            return combined_audio_bytes
        # Normal return with the direct ElevenLabs audio payload.
        return raw_audio
    ...
    def stream_audio(
        ...
        selected_voice: AIVoiceSelectionElevenLabs = (
    ...
    def _stream_audio_bytes(
        ...
        *,
        ...
        """
        Execute one ElevenLabs streaming TTS request and return the aggregated audio bytes.

        Args:
            text_to_convert: Input text that should be synthesized into speech.
            voice: Resolved ElevenLabs voice used for the streaming request.

        Returns:
            Combined audio bytes aggregated from the streaming ElevenLabs response.
        """
        model_id: str = (
            self.selected_model.name if self.selected_model else self.default_model_id
        )
        ...
        combined_audio_bytes: bytes = b"".join(audio_stream)
        if not is_hex_enabled():
            stream(iter([combined_audio_bytes]))
    ...
    def play(self, audio_bytes: bytes) -> None:
        ...
        if not is_hex_enabled():
            ...
            _LOGGER.info("Hex support is enabled; skipping playback.")
